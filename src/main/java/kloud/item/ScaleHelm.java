...
public class ScaleHelm extends ScaleArmor {
	
	private int count = 0;

	public ScaleHelm(ArmorMaterial materialIn, int renderIndexIn, EntityEquipmentSlot equipmentSlotIn, String name) {
...
// ./gradlew runClient
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
		...
		List<PotionEffect> effects = KGUtils.getScaleEffects();
		for(ItemStack is : armors) {
			if(!is.isEmpty() && ((ItemArmor)is.getItem()).getArmorMaterial() == KG.ArmorMaterials.SCALE) {
				count++;
			}
		}
